...
const Profile = () => {
  ...
  const handleSave = () => {
    ...
    const regKey = user.userType === 'farmer' ? 'registeredFarmers' : 'registeredUsers';
    const list = JSON.parse(localStorage.getItem(regKey)) || [];
    const updatedList = list.map(u => u.name === user.name ? updatedUser : u);
      ...
      {editMode ? (
        <>
          ...
          <input
            ...
            placeholder="Email"
          />
          <button onClick={handleSave} className="bg-green-600 text-white px-4 py-1 rounded hover:bg-green-700 mr-2">
            Save
          </button>
          <button onClick={() => setEditMode(false)} className="text-gray-600 hover:underline">
            Cancel
          </button>
        </>
      ) : (
